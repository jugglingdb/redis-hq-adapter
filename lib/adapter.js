    ...
    Object.keys(where).forEach(function (key) {
        ...
        if (this.indexes[model][key]) {
            ...
            if (customSort && kv in customSort ||
                ...
                // allow for multiple indexes for the same key
...
RedisHQ.prototype.all = function all(model, filter, callback) {
    ...
    if (filter.offset) {
        from = parseInt(filter.offset);
    }
    if (filter.limit) {
        to = from + parseInt(filter.limit) - 1;
    }
    ...
    if (!filter.onlyKeys) {
        ...
        this.client.evalsha([rangeCmd, 4, orderedSetName, from, to, redis.modelName(model) + ':'], handleReplies);
    } else {
