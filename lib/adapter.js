    ...
    function save(updatedData, initialData) {
        ...
        hq.client.set([key, JSON.stringify(updatedData)], function (err) {
            ...
            hq.updateIndexes(model, data.id, updatedData, function(err) {
                if (err) return callback(err);
                if (hq.backyard) {
                    hq.client.expire(key, hq.get(model, 'expire') || EXPIRE_DEF);
                    (new hq.backyard.models[model](updatedData)).save(callback);
                } else {
                    callback(err);
                }
            }, initialData);
...
RedisHQ.prototype.destroy = function destroy(model, id, callback) {
    ...
    function done () {
        ...
        if (hq.backyard) {
            (new (hq.backyard[model])({id: id})).destroy();
        }
    }
};
        ...
        function respondWith(replies) {
            ...
            if (redis.backyard) {
                replies.forEach(function(r) {
                    var key = redis.modelName(model) + ':' + r.id;
                    redis.client.expire(key, redis.get(model, 'expire') || EXPIRE_DEF);
                });
            }
