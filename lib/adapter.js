...
RedisHQ.prototype.updateIndexes = function (model, id, data, callback, prevData) {
    ...
    Object.keys(i).forEach(function (key) {
        var effectiveId = id;
        if (sets && sets[key] && prevData) {
            effectiveId = prevData[sets[key]];
        }
...
RedisHQ.prototype.possibleIndexes = function (model, where, orderSuffix) {
    ...
    Object.keys(where).forEach(function (key) {
        ...
        if (sets && sets[key]) {
            res.resetIndex = 'z:' + modelKey + ':$' + key + ':' + val;
        }
...
RedisHQ.prototype.all = function all(model, filter, callback) {
    ...
    var sets = redis.get(model, 'sets');
    var firstKey = Object.keys(filter.where)[0];
    var orderSuffix = this.calcOrderSuffix(model, filter.order);
    var orderedSetName = null;
    var throughModel = sets && filter.collect &&
        filter.collect === filter.include && filter.where && sets[firstKey];
    if (throughModel) {
    ...
    if (filter.limit && (filter.limit > 1 || filter.offset)){
        this.client.zcard(dest, function (err, count) {
    ...
    function handleKeys(err, keys) {
        ...
        if (filter.onlyKeys) return callback(err, keys);

        if (throughModel) {
                console.log('got', keys);
            return redis._models[model].model.include(keys.map(function(k) {
                var obj = {};
                obj[sets[firstKey]] = k;
                return obj;
