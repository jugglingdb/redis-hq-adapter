...
RedisHQ.prototype.backyardExpire = function backyardExpire(model) {
    var hq = this;
    if (!hq.backyard) {
        return false;
    }
    ...
    return function(id) {
        hq.client.expire(hq.modelName(model) + ':' + id, expireSetting);
    }
};
...
RedisHQ.prototype.destroy = function destroy(model, id, callback) {
    ...
    function done () {
        ...
        if (hq.backyard) {
            (new (hq.backyard.models[model])({id: id})).destroy();
        }
    }
};
...
RedisHQ.prototype.all = function all(model, filter, callback) {
    ...
    function handleReplies(keys) {
        ...
        function respondWith(replies) {
            ...
            var expire = redis.backyardExpire(model);
