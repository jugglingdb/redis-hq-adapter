...
RedisHQ.prototype.define = function (descr) {
    ...
    function get(type, args) {
        for (var i = 0; i < args.length; i += 1) {
            var isArr = args[i] instanceof Array;
            if (type === 'array' && isArr) {
                return args[i];
...
RedisHQ.prototype.all = function all(model, filter, callback) {
    ...
    if (filter.where && filter.where.id &&
        ...
        if (filter.limit) {
            filter.where.id.inq = filter.where.id.inq.slice(0, filter.offset + filter.limit);
        }
    ...
    if (!filter.onlyKeys && !throughModel) {
        ...
        if (filter.min && filter.max) {
            command = [rangeCmdHash, 0, orderedSetName, from, to, redis.modelName(model), filter.min, filter.max];
        }
