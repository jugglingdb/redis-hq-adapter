    ...
    Object.keys(where).forEach(function (key) {
        ...
        if (this.indexes[model][key]) {
            ...
            if (customSort && kv in customSort ||
                ...
                if ('object' === typeof val && val.length > 1) {
...
RedisHQ.prototype.perform = function (model, filter, iterator, callback) {
    var self = this;
    var batchSize = 100;
    var index = 0;
    var count = -1;
    ...
    async.whilst(function () { return count !== 0 }, function (callback) {
        filter.offset = index;
        filter.limit = batchSize;

        index += batchSize;

        self.all(model, filter, function (err, results) {
            async.forEach(results, function (result, done) {
                iterator(result, done);
            }, function (err) {
...
RedisHQ.prototype.all = function all(model, filter, callback) {
    ...
    if (!filter.onlyKeys && !throughModel) {
        ...
        this.client.evalsha([rangeCmdHash, 0, orderedSetName, from, to, redis.modelName(model)], handleReplies);
    } else {
