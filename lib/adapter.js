...
RedisHQ.prototype.all = function all(model, filter, callback) {
    ...
    // fulltext
    if (filter.fulltext) {
        return redis.fulltext(model, filter, handleKeys);
    }
    ...
    if (filter.where) {
        ...
        if (indexes && indexes.length || filter.where.id) {
            if (filter.where.id) {
                ...
                client.zadd([dest, filter.where.id, filter.where.id]);
            }
            if (noIndexes.length) {
