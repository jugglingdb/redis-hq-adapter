...
RedisHQ.prototype.create = function (model, data, callback) {
    ...
    function create(id, upsert) {
        ...
        this.client.sadd(["s:" + this.modelName(model), upsert ? data : data.id]);
    }
};
...
RedisHQ.prototype.destroy = function destroy(model, id, callback) {
    ...
    function done () {
        ...
        hq.client.zrem(["z:" + hq.modelName(model), id]);
    }
};

RedisHQ.prototype.possibleIndexes = function (model, filter, orderSuffix) {
    ...
    Object.keys(filter.where).forEach(function (key) {
        ...
        if (typeof val === 'object' && val.inq) {
            val.inq.forEach(function(id) {
                unions.push('z:' + modelKey + ':' + key + ':' + id);
            });
            return;
        }
        ...
        if (typeof val === 'object' && val.lua && typeof val.value !== 'undefined') {
            ...
            luaIndex.push(val);
        } else if (key !== 'id' && this.indexes[model][key]) {
            ...
            if (customSort && kv in customSort ||
                ...
                foundIndex.unshift('z:' + modelKey + ':' + key + ':' + pk + val);
                gotSort = true;
            } else {
                foundIndex.push('z:' + modelKey + ':' + key + ':' + pk + val);
            }
        } else {
            if (key !== 'id') {
                noIndex.push(key);
    ...
    if (!gotSort) {
        foundIndex.unshift('z:' + modelKey + orderSuffix);
    }

    return [foundIndex, luaIndex, noIndex, unions];
};
...
RedisHQ.prototype.all = function all(model, filter, callback) {
    ...
    if (filter.where) {
        ...
        var pi = this.possibleIndexes(model, filter, orderSuffix);
        var indexes = pi[0];
        var luaIndexes = pi[1];
        var noIndexes = pi[2];
        var unions = pi[3];

        if (filter.where.id && typeof filter.where.id !== 'object') {
            filter.where.id = {inq: [filter.where.id]};
        }

        if (indexes.length || unions.length || filter.where.id && filter.where.id.inq) {
            if (filter.where.id && filter.where.id.inq) {
                indexes.push(dest);
                var ids = [dest];
                filter.where.id.inq.forEach(function(id) {
                    ids.push(id);
                    ids.push(id);
                });
                client.zadd(ids);
            ...
            if (unions.length) {
                indexes.push(dest);
                client.zunionstore([dest, unions.length + 1, dest].concat(unions));
            }
            if (noIndexes.length) {
                throw new Error('No indexes for ' + noIndexes.join(', '));
            }
            usingTempList = true;
            var numKeys = indexes.length;
            indexes.unshift(numKeys);
            indexes.unshift(dest);
            var weights = ['WEIGHTS', 1];
            for (var keyIndex = 1; keyIndex < numKeys; keyIndex += 1) {
                weights.push(0);
            }
            client.zinterstore(indexes.concat(weights));
            client.expire([dest, 7]); // TODO: replace with DEL dest
            ...
            if (luaIndexes.length > 0) {
                ...
                luaIndexes.forEach(function(index) {
                    ...
                    if(!scriptSha) {
                        ...
                        client.eval([script, 4, key, dest, index.value, index.limit || 0]);
                        ...
                        client.evalsha([scriptSha, 4, key, dest, index.value, index.limit || 0]);
                    }
    ...
    if (filter.limit && (filter.limit > 1 || filter.offset)){
        client.zcard(dest, function (err, count) {
    ...
    if (!usingTempList) {
        dest = 'z:' + modelKey + orderSuffix;
    }
    client[rangeCmd]([dest, filter.offset || 0, (filter.limit || 0) - 1], handleKeys);
    ...
    function handleKeys(err, keys) {
        ...
        redis.client.mget(query, function (err, replies) {
