...
RedisHQ.prototype.transaction = function () {
    ...
    ['sinterstore', 'sadd', 'expire', 'sort', 'scard', 'smembers', 'keys', 'eval', 'evalsha'].forEach(function (k) {
...
RedisHQ.prototype.updateIndexes = function (model, id, data, callback, prevData) {
    ...
    var p = this._models[model].properties;
    var schedule = [['SADD', 's:' + this.modelName(model), id]];
    ...
    Object.keys(i).forEach(function (key) {
        ...
        if (typeof data[key] === 'undefined' || prevData && prevVal !== curVal) {
            ...
            schedule.push([
                'DEL',
                's:' + redis.modelName(model) + ':' + id + ':' + key
            ]);
        }
    });
    ...
    Object.keys(data).forEach(function (key) {
        if (i[key]) {
            ...
            //only push the first value into the sort set
            if(val.length > 0) {
                schedule.push([
                    'SET',
                    's:' + redis.modelName(model) + ':' + id + ':' + key,
...
RedisHQ.prototype.destroy = function destroy(model, id, callback) {
    ...
    function done () {
        ...
        hq.srem(["s:" + hq.modelName(model), id]);
    }
};
...
RedisHQ.prototype.possibleIndexes = function (model, filter) {
    ...
    Object.keys(filter.where).forEach(function (key) {
        ...
        if(typeof val === 'object' && val.lua && typeof val.value !== 'undefined') {
            ...
            foundIndex.push('i:' + this.modelName(model) + ':' + key + ':' + pk + val);
        } else {
            if (key !== 'id') {
                noIndex.push(key);
            }
...
RedisHQ.prototype.all = function all(model, filter, callback) {
    ...
    if (filter && filter.where) {
        ...
        if (indexes && indexes.length || filter.where.id) {
            if (filter.where.id) {
                ...
                transaction.sadd([dest].concat(filter.where.id));
            }
            if (noIndexes.length) {
            ...
            if (indexes.length > 1 || indexes.length === 1 && filter.intersect) {
                ...
                transaction.sinterstore(indexes);
    ...
    if (filter && filter.order) {
        ...
        orders.forEach(function (key) {
            ...
            var nestKey = modelIndexes[key.replace(/^\d+:/, '')];
            if (key !== 'id') {
                if (
                    propKey && propKey.type.name !== 'Number' && propKey.type.name !== 'Date' ||
                    nestKey && nestKey[0] && nestKey[0].name !== 'Number' && nestKey[0].name !== 'Date'
                ) {
                    allNumeric = false;
                    console.log(propKey, nestKey);
                }
            }
            sortCmd.push('BY', 's:' + redis.modelName(model) + ':*:' + key);
        });
    }
    ...
    if (sortCmd.length) {
        sortCmd.unshift(usingTempList ? dest : ("s:" + redis.modelName(model)));
        sortCmd.push("GET", "#");
        transaction.sort(sortCmd, handleKeys);
    } else {
        ...
        if (!usingTempList) {
            if (ft && ft.length) {
                ...
                transaction.smembers('s:' + redis.modelName(model), handleKeys);
            }
        } else {
            transaction.smembers(dest, handleKeys);
        }
    }
    ...
    function handleKeys(err, keys) {
        if (!keys || !keys.length) return callback(err, []);
...
RedisHQ.prototype.destroyAll = function destroyAll(model, callback) {
    ...
    this.keys(keysQuery, function (err, keys) {
        ...
        redis.multi(query, function (err, replies) {
            redis.del('s:' + redis.modelName(model), function () {
                callback(err);
            });
        });
    });
